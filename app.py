...
# ============================================================
if "mostrar_parametros" not in st.session_state:
    st.session_state.mostrar_parametros = True  # começa visível
...
# ============================================================
# LAYOUT COM DUAS COLUNAS (PARÂMETROS + RESULTADOS)
# ============================================================
col_esq, col_dir = st.columns([0.3, 0.7] if st.session_state.mostrar_parametros else [0, 1])

# ---- COLUNA ESQUERDA (PARÂMETROS) ----
with col_esq:
    if st.session_state.mostrar_parametros:
        ...
        # ICTs
        st.subheader("🏛️ Instituições Científicas (ICTs)")
        ict_selecionadas = st.multiselect(
        ...
        with col_botao1:
            simular = st.button("🔄 Simular Cenário", type="primary", use_container_width=True)
        with col_botao2:
            ocultar = st.button("⬅️ Ocultar Parâmetros", use_container_width=True)
            if ocultar:
                st.session_state.mostrar_parametros = False
                st.rerun()

        # Expansores com explicações
        with st.expander("📘 O que é uma Coexecutora?"):
            st.markdown("""
        ...
        with st.expander("📘 O que é uma ICT?"):
            st.markdown("""
        ...
        with st.expander("📘 O que é Parceiro Social?"):
            st.markdown("""
        ...
        with st.expander("📘 O que é ROB (Receita Operacional Bruta)?"):
            st.markdown("""
        ...
        st.caption("**Referências:** Regulamento, Anexo 1, FAQ e Apresentação da chamada.")

    else:
        # Se o painel estiver oculto, mostrar botão para reabrir
        if st.button("➡️ Mostrar Parâmetros", use_container_width=True):
            st.session_state.mostrar_parametros = True
            st.rerun()

# ---- COLUNA DIREITA (RESULTADOS) ----
with col_dir:
    # Se o painel de parâmetros estiver oculto, ocupar 100% da largura
    if not st.session_state.mostrar_parametros:
        # Ajuste visual para ocupar toda a largura
        st.markdown("""
        <style>
            .stColumn { padding: 0; }
        </style>
    ...
    # ============================================================
    # DICIONÁRIOS E FUNÇÕES (COLOCADOS AQUI PARA ACESSO)
    # ============================================================
    contrapartida_table = {
    ...
    # Verificar se os parâmetros foram definidos (caso o painel esteja oculto, os valores ainda existem)
    if "porte" not in locals() or "arranjo" not in locals():
        # Se não houver parâmetros definidos (primeira execução), mostrar instrução
    ...
    if simular:
        ...
        # Se não houver simulação, mostrar mensagem inicial
        st.info("👈 **Preencha os parâmetros no painel lateral e clique em 'Simular Cenário'** para visualizar os resultados e comparar os arranjos.")
        st.markdown("""
        ### O que você vai encontrar aqui:
        - **Comparação lado a lado** dos dois arranjos (Simples vs Rede).
...
# ============================================================
# RODAPÉ FIXO (exibido na coluna direita ou abaixo)
# ============================================================
st.sidebar.markdown("---")
